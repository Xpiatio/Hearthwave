...
**ICS-214 export (for ARES/RACES):** With a net selected, click **ICS-214 (CSV)** next to **DOWNLOAD CSV** to export that net as an ICS-214 Activity Log. Four boxes of the form have no counterpart in a net record, so a dialog asks for them first: **incident name** and **prepared by** are required, **ICS position** (default *Net Control Station*) and **home agency (and unit)** are optional. They're remembered in this browser and pre-filled next time, so a multi-net activation only types them once.
