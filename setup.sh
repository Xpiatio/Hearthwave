#!/usr/bin/env bash
# Hearthwave one-shot setup — run once from the repo root, then:
...
if [[ ! -f "backend/requirements.txt" ]]; then
  echo "Error: run this script from the Hearthwave repo root." >&2
  echo "  cd /path/to/Hearthwave && bash setup.sh" >&2
  exit 1
fi
...
if [[ -f ".env" ]]; then
  ...
  cat > .env <<EOF
# Hearthwave Docker Compose environment — generated by setup.sh
